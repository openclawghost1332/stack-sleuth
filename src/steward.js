...
export function describeCasebookStewardHeadline(report) {
  const headline = String(report?.summary?.headline ?? 'No steward summary available.');
  return report?.preserved === false
...
export function renderCasebookStewardText(report) {
  return [
    ...
    `Headline: ${describeCasebookStewardHeadline(report)}`,
    ...(report.preserved === false ? ['Replay note: Stewardship was reconstructed from older dataset fields.'] : []),
    '',
...
export function renderCasebookStewardMarkdown(report) {
  return [
    ...
    `- **Actions:** ${report.summary.actionCount}`,
    `- **Urgent actions:** ${report.summary.urgentActionCount}`,
    `- **Headline:** ${escapeMarkdownText(describeCasebookStewardHeadline(report))}`,
    ...(report.preserved === false ? ['- **Replay note:** Stewardship was reconstructed from older dataset fields.'] : []),
    '',
    '### Top stewardship actions',
